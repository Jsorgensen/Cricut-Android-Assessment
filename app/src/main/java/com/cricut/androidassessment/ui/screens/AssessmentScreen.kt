...
fun QuizContent(
    modifier: Modifier = Modifier,
    uiState: QuizUiState,
    onAnswerSelected: (questionId: String, answer: Boolean) -> Unit
) {
    if (uiState.currentQuestionIndex >= uiState.questions.size) {
        Text("Quiz finished or error in question index.", modifier = modifier.padding(16.dp))
        return
    }
    ...
    Column(
        ...
        Row(
            ...
            Text("(Next/Back buttons placeholder)")
        }
    }
}
